/**
 * @fileoverview Operation IBAMACertidaoNegativaDebitos — DirectData Marketplace API.
 * Endpoint para realizar a consulta IBAMA - Certidão Negativa de Débitos.
 * @module infrastructure/providers/directdata/operations/IBAMACertidaoNegativaDebitos
 */

import { isLeft } from '@shared/domain/Either.js';
import type { Either } from '@shared/domain/Either.js';
import type { SourceError } from '@shared/domain/errors/SourceError.js';
import { parseOrSchemaError } from '@shared/domain/parseOrSchemaError.js';
import type { IHttpClient } from '@shared/infrastructure/IHttpClient.js';
import { z } from 'zod';
import { DirectDataMetaDadosSchema } from '../dtos/DirectDataResponseDto.js';
import { IBAMACertidaoNegativaDebitosRetornoSchema } from '../dtos/IBAMACertidaoNegativaDebitosDto.js';
import type { IIBAMACertidaoNegativaDebitos } from '../ports/IIBAMACertidaoNegativaDebitos.js';

const ResponseSchema = z.object({
  metaDados: DirectDataMetaDadosSchema,
  retorno: IBAMACertidaoNegativaDebitosRetornoSchema.nullable(),
});

/**
 * Operation para endpoint `IBAMACertidaoNegativaDebitos`.
 *
 * @class IBAMACertidaoNegativaDebitos
 * @implements {IIBAMACertidaoNegativaDebitos}
 */
export class IBAMACertidaoNegativaDebitos implements IIBAMACertidaoNegativaDebitos {
  readonly path = '/api/IBAMACertidaoNegativaDebitos';

  constructor(private readonly http: IHttpClient) {}

  async execute(
    params: Record<string, string | undefined>,
  ): Promise<Either<SourceError, ReturnType<typeof ResponseSchema.parse>>> {
    const cleanParams: Record<string, string> = {};
    for (const [key, value] of Object.entries(params)) {
      if (value !== undefined && value !== '') {
        cleanParams[key] = value;
      }
    }

    const result = await this.http.request<unknown>(this.path, {
      method: 'GET',
      params: cleanParams,
    });

    if (isLeft(result)) return result;

    return parseOrSchemaError(ResponseSchema, result.value, 'directdata');
  }
}
