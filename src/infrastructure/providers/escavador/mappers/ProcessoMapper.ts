...
export class ProcessoMapper {
  ...
  toInternal(dto: ProcessoDto): ProcessoInternal {
    const result: ProcessoInternal = {
      numeroCnj: dto.numero_cnj ?? dto.numero_novo ?? '',
      tribunal: dto.tribunal ?? '',
      ativo: dto.ativo ?? true,
      partes: (dto.partes ?? []).map((p) => ({ nome: p.nome, tipoParte: p.tipo_parte })),
    };
    if (dto.data_ajuizamento != null) result.dataAjuizamento = dto.data_ajuizamento;
    if (dto.tipo_acao != null) result.tipoAcao = dto.tipo_acao;
    if (dto.valor_causa != null) result.valorCausa = dto.valor_causa;
    if (dto.ultima_movimentacao != null) result.ultimaMovimentacao = dto.ultima_movimentacao;
    return result;
  }
}
