...
import { CallbackV2DtoSchema, type CallbackV2Dto } from '../../dtos/v2/CallbackV2Dto.js';
...
export class ReenviarCallbackV2 implements IReenviarCallbackV2 {
  ...
  async execute(input: { id: number }): Promise<Either<SourceError, CallbackV2Dto>> {
    const result = await this.http.request<unknown>(`/api/v2/callbacks/${input.id}/reenviar`, { method: 'POST' });
