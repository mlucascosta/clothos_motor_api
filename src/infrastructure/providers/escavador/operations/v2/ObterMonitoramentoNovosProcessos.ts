...
export class ObterMonitoramentoNovosProcessos implements IObterMonitoramentoNovosProcessos {
  ...
  async execute(input: { id: number }): Promise<Either<SourceError, MonitoramentoNovosProcessosDto>> {
    const result = await this.http.request<unknown>(`/api/v2/monitoramentos/novos-processos/${input.id}`);
