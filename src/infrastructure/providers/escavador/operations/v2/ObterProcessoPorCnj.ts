...
import { ProcessoV2DtoSchema, type ProcessoV2Dto } from '../../dtos/v2/ProcessoV2Dto.js';
...
export class ObterProcessoPorCnj implements IObterProcessoPorCnj {
  ...
  async execute(input: { numero: string }): Promise<Either<SourceError, ProcessoV2Dto>> {
    const result = await this.http.request<unknown>(`/api/v2/processos/numero_cnj/${encodeURIComponent(input.numero)}`);
