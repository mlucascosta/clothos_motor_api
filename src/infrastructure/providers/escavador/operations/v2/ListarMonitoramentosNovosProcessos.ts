...
import { ListarMonitoramentosNovosProcessosResponseSchema, type ListarMonitoramentosNovosProcessosResponse } from '../../dtos/v2/MonitoramentoV2Dto.js';

export interface IListarMonitoramentosNovosProcessos {
  execute(input: { pagina?: number }): Promise<Either<SourceError, ListarMonitoramentosNovosProcessosResponse>>;
}

export class ListarMonitoramentosNovosProcessos implements IListarMonitoramentosNovosProcessos {
  constructor(private readonly http: IHttpClient) {}

  async execute(input: { pagina?: number }): Promise<Either<SourceError, ListarMonitoramentosNovosProcessosResponse>> {
    const params: Record<string, string | number | boolean | undefined> = {};
    if (input.pagina !== undefined) params['page'] = input.pagina;

    const result = await this.http.request<unknown>('/api/v2/monitoramentos/novos-processos', { params });
