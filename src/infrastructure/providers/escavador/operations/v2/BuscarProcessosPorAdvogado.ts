...
export class BuscarProcessosPorAdvogado implements IBuscarProcessosPorAdvogado {
  ...
  async execute(input: { oab: string; pagina?: number }): Promise<Either<SourceError, ProcessoV2Resumo>> {
    const params: Record<string, string | number | boolean | undefined> = {};
    if (input.pagina !== undefined) params['page'] = input.pagina;

    const result = await this.http.request<unknown>(`/api/v2/processos/advogado/${encodeURIComponent(input.oab)}`, { params });
