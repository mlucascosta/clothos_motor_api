...
import { AutenticacaoDtoSchema, type AutenticacaoDto } from '../../dtos/v2/CertificadoDto.js';

export interface ICriarAutenticacaoCertificado {
  execute(input: { id: number; tipo: string; valor?: string }): Promise<Either<SourceError, AutenticacaoDto>>;
}

export class CriarAutenticacaoCertificado implements ICriarAutenticacaoCertificado {
  constructor(private readonly http: IHttpClient) {}

  async execute(input: { id: number; tipo: string; valor?: string }): Promise<Either<SourceError, AutenticacaoDto>> {
    const body: Record<string, unknown> = { tipo: input.tipo };
    if (input.valor !== undefined) body['valor'] = input.valor;

    const result = await this.http.request<unknown>(`/api/v2/certificados/${input.id}/autenticacoes`, {
      method: 'POST',
      body,
    });
