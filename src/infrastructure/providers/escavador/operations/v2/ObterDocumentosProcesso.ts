...
import { DocumentosV2ResponseSchema, type DocumentosV2Response } from '../../dtos/v2/ProcessoV2Dto.js';

export interface IObterDocumentosProcesso {
  execute(input: { numero_cnj: string; pagina?: number }): Promise<Either<SourceError, DocumentosV2Response>>;
}

export class ObterDocumentosProcesso implements IObterDocumentosProcesso {
  constructor(private readonly http: IHttpClient) {}

  async execute(input: { numero_cnj: string; pagina?: number }): Promise<Either<SourceError, DocumentosV2Response>> {
