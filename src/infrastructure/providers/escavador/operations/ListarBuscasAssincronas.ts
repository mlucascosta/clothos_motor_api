...
export interface IListarBuscasAssincronas {
  execute(input: { pagina?: number }): Promise<Either<SourceError, ListarBuscasAssincronasResponse>>;
}

export class ListarBuscasAssincronas implements IListarBuscasAssincronas {
  constructor(private readonly http: IHttpClient) {}

  async execute(input: { pagina?: number }): Promise<Either<SourceError, ListarBuscasAssincronasResponse>> {
