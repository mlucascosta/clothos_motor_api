...
export interface IListarMonitoramentos {
  execute(input: { pagina?: number; ativo?: boolean }): Promise<Either<SourceError, ListarMonitoramentosResponse>>;
}

export class ListarMonitoramentos implements IListarMonitoramentos {
  constructor(private readonly http: IHttpClient) {}

  async execute(input: { pagina?: number; ativo?: boolean }): Promise<Either<SourceError, ListarMonitoramentosResponse>> {
