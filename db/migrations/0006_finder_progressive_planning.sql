...
CREATE TABLE IF NOT EXISTS reduto_core.job_events (
  id          BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
  job_id      UUID NOT NULL REFERENCES reduto_core.jobs(job_id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_job_events_job_sequence
  ON reduto_core.job_events (job_id, sequence);

CREATE TABLE IF NOT EXISTS reduto_core.job_source_executions (
  id            BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
  job_id        UUID NOT NULL REFERENCES reduto_core.jobs(job_id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_job_source_executions_job_stage
  ON reduto_core.job_source_executions (job_id, stage, id);

CREATE TABLE IF NOT EXISTS reduto_core.derived_artifacts (
  id                    BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
  job_id                UUID NOT NULL REFERENCES reduto_core.jobs(job_id) ON DELETE CASCADE,
  artifact_key          TEXT NOT NULL,
  value                 JSONB NOT NULL,
  provenance            JSONB NOT NULL,
  source_execution_id   BIGINT NOT NULL REFERENCES reduto_core.job_source_executions(id),
  created_at            TIMESTAMPTZ NOT NULL DEFAULT now(),
...
CREATE INDEX IF NOT EXISTS idx_derived_artifacts_job
  ON reduto_core.derived_artifacts (job_id, artifact_key);
