...
-- ---------------------------------------------------------------------------
-- reduto_core.jobs — índices adicionais (idx_jobs_claim já criado em 0001)
-- ---------------------------------------------------------------------------
...
-- Query: SELECT … FROM reduto_core.jobs WHERE status='failed' ORDER BY updated_at DESC
CREATE INDEX IF NOT EXISTS idx_jobs_dlq
  ON reduto_core.jobs (updated_at DESC)
...
-- Query: SELECT … FROM reduto_core.jobs WHERE correlation_id = $1
CREATE INDEX IF NOT EXISTS idx_jobs_correlation_id
  ON reduto_core.jobs (correlation_id);
...
-- Query: SELECT … FROM reduto_core.jobs WHERE tenant_slug = $1 ORDER BY created_at DESC
CREATE INDEX IF NOT EXISTS idx_jobs_tenant_created
  ON reduto_core.jobs (tenant_slug, created_at DESC);
...
-- Query: SELECT … FROM reduto_core.jobs WHERE status='claimed' AND claimed_at < now() - interval
CREATE INDEX IF NOT EXISTS idx_jobs_claimed_stale
  ON reduto_core.jobs (claimed_at)
  WHERE status = 1;   -- claimed

-- ---------------------------------------------------------------------------
-- reduto_core.raw_results
-- ---------------------------------------------------------------------------
...
CREATE INDEX IF NOT EXISTS idx_raw_results_gateway_created
  ON reduto_core.raw_results (gateway, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_raw_results_param_lookup
  ON reduto_core.raw_results (tipo_param, param, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_raw_results_status_created
  ON reduto_core.raw_results (status, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_raw_results_correlation_id
  ON reduto_core.raw_results (correlation_id);
...
CREATE INDEX IF NOT EXISTS idx_raw_results_result_gin
  ON reduto_core.raw_results USING gin (result jsonb_path_ops);

-- ---------------------------------------------------------------------------
-- reduto_core.query_refs
-- ---------------------------------------------------------------------------
...
CREATE INDEX IF NOT EXISTS idx_query_refs_tenant_created
  ON reduto_core.query_refs (tenant_id, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_cache_expires_at
  ON reduto_core.cache (expires_at);

-- ---------------------------------------------------------------------------
-- reduto_core.providers
-- ---------------------------------------------------------------------------
